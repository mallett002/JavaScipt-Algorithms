...
describe('coin-change', () => {
    ...
    it('should return -1 when coins = [8], amount = 1', () => {
        const result = retrieveFewestCoins([8], 1);

        expect(result).toBe(-1);
    });

    it('should return 3 when coins = [1], amount = 1', () => {
        const result = retrieveFewestCoins([1], 1);

        expect(result).toBe(1);
    });
    
    it('should return 3 when coins = [1,2,5], amount = 3', () => {
        const result = retrieveFewestCoins([1, 2, 5], 3);
